...
/**
 * tooling to analyze compound object storage files could be found here:
...
public class OutlookItemTransferImpl extends OutlookItemTransfer {
	...
	private String getFilename(long fileDescriptorPointer) {
		...
		int firstZero = filename.indexOf('\0');
		if (firstZero > 0)  {
			filename = filename.substring(0, firstZero);
		}
	...
	private byte[] getFileContents(int fileIndex, IDataObject data) {
		...
		if (data.GetData(format, medium) == COM.S_OK) {
			switch (medium.tymed) {
				...
				case TYMED_ISTREAM: {
					CompoundStream stream = CompoundFactory.createFromIStream(medium.unionField);
					if (stream != null) {
